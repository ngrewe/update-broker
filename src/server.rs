...
pub fn engine(path: &Path, logger: Rc<Logger>) -> Result<(),IoError> {
    ...
    if connection_r.is_err() {
        return connection_r.map(|_| ())
            .map_err(|e| IoError::new(ErrorKind::Other, format!("Error creating connection. {:?}", e)))
    }
    ...
    if registration.is_err() {
        return registration.map(|_| ())
            .map_err(|e| IoError::new(ErrorKind::Other, format!("Error registering name. {:?}", e)))
    }
    ...
    let tree = f.tree(ATree::new()).add(f.object_path("/com/coreos/update1/Manager", ()).introspectable().add(
        f.interface("com.coreos.update1.Manager", ())
            ...
            .add_m(f.method("GetStatus", (), move |m| {
                ...
                debug!(&l, "Sending update status to {:?}: {:?}", m.msg.sender(), status);
    ...
    if registration2_r.is_err() {
        return registration2_r.map_err(|e| IoError::new(ErrorKind::Other, format!("Error registering D-Bus tree. {:?}", e)))
    }
    let core_r = Core::new();
    if core_r.is_err() {
        return core_r.map(|_| ());
    ...
    let server = server.for_each(|m| { debug!(&logger, "Unhandled message: {:?}", m); Ok(()) });
    ...
    let ex = core.execute(notifier.unwrap().map_err(move |e| {
        ...
        Timer::default().sleep(time::Duration::from_millis(200)).wait().unwrap();
        process::exit(1);
    }));
    if ex.is_err() {
        return ex.map(|_| ()).map_err(|e| 
            IoError::new(ErrorKind::Other, format!("Could not schedule inotify watcher. {:?}", e)))
    }
    ...
    return core.run(termination).map(|_| ()).map_err(|e| IoError::new(ErrorKind::Other, "Error running server"));
    // Ok(())
}
